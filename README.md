...
当前目录版本为 `0.3.0`。当前可直接在本机 macOS ARM 上测试；Intel Mac 和 Windows 发行目录仍需填充对应架构的 Python 运行时。GUI 顶部提供“检查更新”按钮；更新模块支持本地或 HTTPS manifest、平台包下载、SHA-256 校验、保留用户数据目录和失败回滚。当前远程更新清单为：
